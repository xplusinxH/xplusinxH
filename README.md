<div align="center">

# xplusinxH

### ROBOTICS SYSTEMS | PERCEPTION | MOTION | HARDWARE

<p>
  hi, i'm xplusinxH, a junior undergraduate. I foucs on build robots from mechanical structure to intelligent motion.
</p>

<p>
  <a href="#mission-profile">Mission Profile</a> |
  <a href="#engineering-toolkit">Engineering Toolkit</a> |
  <a href="#competition-record">Competition Record</a>
</p>

</div>

---

## Mission Profile

I am a full-stack robotics developer focused on turning perception into
reliable physical action.My work connects **robot vision**, **motion control**
, **mechanical design**,and **embedded hardware** into complete robotic systems.

## Current Coordinates

```yaml
focus:
  - robotic perception and vision-guided control
  - motion planning and control systems
  - mechanical structure and hardware integration
  - embedded and real-time robotics

engineering_style:
  - system-level thinking
  - rapid prototyping
  - competition-tested execution
  - hands-on debugging
```

## Engineering Toolkit

| System Layer | Technologies |
| --- | --- |
| **Mechanical & Electronic Design** | SolidWorks / Ansys / Altium Designer / LCEDA |
| **Robotics & Simulation** | ROS / ROS 2 / Gazebo / Isaac Gym / MoveIt / MATLAB |
| **Software & Realtime** | C / C++ / Python / Linux / FreeRTOS / Git / CMake |
| **Embedded Compute** | STM32 / NVIDIA Jetson |

## Competition Record

### 2026

**VEX Asia Open - First Prize**  

### 2025

**24th ROBOCON "Flying Dunk" Competition - National Top 8**  
**Volleyball Challenge - National Champion**  
**Bionic Legged Robot Off-Road & Racing Competition - National Second Prize**

### 2024

**RoboCup China Competition, Advanced Vision - National First Prize**  


<div align="center">

### BUILD | TEST | ITERATE | COMPETE

## Connect With Me

[![X](https://img.shields.io/badge/X-@xplusin-000000?style=for-the-badge&logo=x&logoColor=white)](https://x.com/xplusin)
